function [rankA, U_hat, S_hat, V_hat_dot, E_hat,numIter] = svd_rpca(D, lambda,...
...
%
% [U_hat, S_hat, V_hat_dot, E_hat] - estimates for the low-rank part and error part, respectively
...
while ~converged
    ...
    if ~lineSearchFlag
        ...
        diagS = diag(S) ;
        S_diag = diag(pos(diagS-mu_k/tau_k));
        ...
        while ~convergedLineSearch
            ...
            [U,S,V] = svd(G_A,'econ') ;
            diagS = diag(S) ;
            S_diag = diag(pos(diagS-mu_k/tau_hat));
            SG_A = U * S_diag * V';
...
U_hat = U(:,1:rankA) ;
S_hat = S_diag(1:rankA,1:rankA);
V_hat_dot = V';
V_hat_dot = V_hat_dot(1:rankA,:);
E_hat = X_k_E;
